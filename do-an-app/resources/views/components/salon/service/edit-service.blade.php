                                ...
                                <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control nice-select sort-select" name="cse_id" id="cse_id">
                                                ...
                                                @foreach($categoryServices as $cs)
                                                    <option value="{{$cs->cse_id}}" {{$service->cse_id == $cs->cse_id ? 'selected' : ''}}>{{$cs->name}}</option>
                                                @endforeach
                                            </select>
                                            <div class="invalid-feedback message-error"></div>
                                        </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" name="description" rows="10" placeholder="Miêu tả">{{$service->description}}</textarea>
                                            <div class="invalid-feedback message-error"></div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <div id="service-picture-container">
                                                <img id="service-picture" src="/media/service/{{$service->image}}">
                                            </div>
...
@push('scripts')
    <script type="module">
        ...
        handleConfirmModal(()=> {
            $.ajax({
                type: "POST",
                url: "{{route('salon.service.update')}}",
                data: formData,
                processData: false,
                contentType: false,
        ...
        $("#saveBtn").on("click", function(event) {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('salon.service.validateUpdate')}}",
                data: formData,
                processData: false,
                contentType: false,
                beforeSend: function () {
